...
import com.smartsoft.repository.AuthorityRepository;
...
import com.smartsoft.security.AuthoritiesConstants;
...
public class UserService {
    ...
    public User createUser(String mobile, String password) {
        ...
        newUser.setLogin(mobile);
        newUser.setActivated(true);
        newUser.setMobile(mobile);
        // new user gets initially a generated password
        newUser.setPassword(encryptedPassword);
        newUser.setFirstName(RandomUtil.generateName());
...
//        newUser.setActivationKey(RandomUtil.generateActivationKey());
    ...
    public void changePassword(String mobile, String password) {
        userRepository.findOneByLogin(mobile).ifPresent(user -> {
            String encryptedPassword = passwordEncoder.encode(password);
            user.setPassword(encryptedPassword);
            log.debug("Changed password for User: {}", user);
        });
    ...
    public List<User> getAllUsersByRoleNameAndTenantId(String roleName,Long tenantId) {
        return userRepository.getAllUsersByRoleNameAndTenantId(roleName,tenantId);
    }
     ...
     */
    @Transactional(readOnly = true)
    public Page<UserDTO> getAllUsersByQuery(String query, Pageable pageable) {
        String queryLike = query + "%";
        return userRepository.findAllByFirstNameLikeOrEmailLikeOrLoginLikeAndTenantId(pageable,
            queryLike, queryLike, queryLike, SecurityUtils.getCurrentUserTenantId()).map(UserDTO::new);
    }
    ...
    public User getUserWithAuthorities() {
        ...
        if(user != null){
            user.getAuthorities().stream().forEach(role -> {
                Role newRole = roleRepository.findOneWithEagerRelationships(role.getId());
