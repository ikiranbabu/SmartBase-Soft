...
import com.smartsoft.service.CodeService;
...
public class CodeResource {
     ...
     * @return the ResponseEntity with status 200 (OK) and the list of codes in body
     */
    @GetMapping("/codes/group-code")
    @Timed
    public ResponseEntity<List<CodeDTO>> getCodesByGroupCode(@RequestParam(name = "groupCode") String groupCode) {
        log.debug("REST request to get a page of Codes");
        List<CodeDTO> result = codeService.findByGroupCode(groupCode);
        return new ResponseEntity<>(result, HttpStatus.OK);
    }


    @GetMapping("/codes/{code}/{codeGroupCode}")
    @Timed
    public ResponseEntity<CodeDTO> getCodes(@PathVariable String code,
                                            @PathVariable String codeGroupCode)
        throws URISyntaxException {
        log.debug("REST request to get a page of Code code: {code}, and codeGroupCode: " + codeGroupCode);
        CodeDTO codeDTO = codeService.findByCodeAndCodeGroupCode(code, codeGroupCode);
