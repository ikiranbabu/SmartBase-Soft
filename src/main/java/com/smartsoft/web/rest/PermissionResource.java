...
public class PermissionResource {
    ...
    public ResponseEntity<Void> deletePermission(@PathVariable Long id) {
        ...
        permission.setId(id);
        List<Role> list = roleRepository.findByPermissionsAndTenantId(permission, SecurityUtils.getCurrentUserTenantId());
        if(list.size() > 0) {
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert(ENTITY_NAME, "permissionExits", "you have used it in permission")).body(null);
        } else {
