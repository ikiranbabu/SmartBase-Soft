...
@Component({
    selector: 'smartsoft-permission-group',
...
currentAccount: any;
    ...
    constructor(
        ...
        // this.jhiLanguageService.setLocations(["permissionGroup"]);
    }
    ...
    transition() {
        this.router.navigate(['/app/acc-ctrl/permission-group'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/app/acc-ctrl/permission-group', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: PermissionGroup) {
        return item.id;
    }
    registerChangeInPermissionGroups() {
        this.eventSubscriber = this.eventManager.subscribe('permissionGroupListModification', (response) => this.loadAll());
    }
